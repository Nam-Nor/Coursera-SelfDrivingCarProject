    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                circle_locations[:, 1] = np.add(path[1][j], np.multiply(self._circle_offsets, np.cos(path[2][j])) )
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                path = paths[i]
                x_path = path[0][len(path) - 1]
                y_path = path[1][len(path) - 1]
                score = np.linalg.norm(np.subtract([x_path, y_path],[goal_state[0], goal_state[1]]))
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            # --------------------------------------------------
                            collision = paths[j]
                            x_collision = collision[0][len(collision) - 1]
                            y_collision = collision[1][len(collision) - 1]
                            score += abs(j-i)# np.linalg.norm(np.subtract([x_path, y_path],[x_collision, y_collision]))
                            # --------------------------------------------------
