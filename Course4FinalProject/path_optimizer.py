...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        term2 = np.multiply(np.divide(b,2),np.power(s,2))
        term3 = np.multiply(np.divide(c,3),np.power(s,3))
        term4 = np.multiply(np.divide(d,4),np.power(s,4))
        thetas =  term1 + term2 + term3 + term4 
        return thetas
        # ------------------------------------------------------------------
