...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)[1] # returns only second function return value
                self._goal_index = goal_index
                self._goal_state = waypoints[goal_index]
                # --------------------------------------------------------------
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            wp1 = np.array( ( waypoints[wp_index][0], waypoints[wp_index][1] ) )
            wp2 = np.array( ( waypoints[wp_index + 1][0], waypoints[wp_index + 1][1] ) )
            arc_length += np.linalg.norm(np.subtract(wp1, wp2))
            wp_index += 1
            if arc_length > self._lookahead:
                break
    ...
    def get_closest_index(self, waypoints, ego_state):
        ...
        for i in range(len(waypoints)):
            current_pos = [waypoints[i][0], waypoints[i][1]]
            dist = np.linalg.norm(np.subtract(current_pos,ego_pos))
            if (dist < closest_len):
                closest_len = dist
                closest_index = i
